...
//--------------------------------------------------
    ...
    SetupInstalledStationPhasedArray();
...
//        params.number_of_surfaces_station = 4;
        ...
        params.number_of_stations = 1;
        params.number_of_strings_station = 1;   // phased array has 1 strings
        params.number_of_antennas_string = 7; // 7 Vpole antennas on one strings
        ...
        if ( ARA_N.is_open() ) {
            while (ARA_N.good() ) {
                ...
                if (line[0] != "/"[0]) {
                    ...
                    else if (label == "z_btw56") {
                        z_btw_array[6] = atof( line.substr( line.find_first_of("=") + 1).c_str() );
                        cout<<"read z_btw bh ant5 and ant6"<<endl;
                    }
//                    else if (label == "z_btw67") {
//                        z_btw_array[7] = atof( line.substr( line.find_first_of("=") + 1).c_str() );
//                        cout<<"read z_btw bh ant4 and ant5"<<endl;
//                    }
        ...
        cout<<"Bore hole Antenna layout is "<<params.bore_hole_antenna_layout<<endl;
        cout<<"Surface stations are "<<params.number_of_surfaces_station<<endl;
        ...
        else if(params.bore_hole_antenna_layout == 5) { // VVVVVVV layour for phased array
            params.number_of_antennas_string = 7;
        }
...
//
                ...
                else if (params.bore_hole_antenna_layout == 5) { //Phased array, 7 vpol antennas

                    for (int j=0; j<params.number_of_strings_station; j++) {
                        cout<<"Number of antennas in string "<<params.number_of_antennas_string<<endl;
                        for (int k=0; k<params.number_of_antennas_string; k++) {
                            ...
                            if (settings1->BH_ANT_SEP_DIST_ON==0)
                                ...
                                stations[i].strings[j].antennas[k].SetZ( -z_max + z_btw_total );
                                cout<<"depth of "<<k<<"th antenna is "<<-z_max + z_btw_total<<endl;
                            }


                            //if (k == 0) {   // only the first antenna is V pol
                                stations[i].strings[j].antennas[k].type = 0;   // all antennas are vpol
                        ...
                        cout<<"End loop of antenna string"<<endl;
                        }
                    }
                }
...
void Detector::SetupInstalledStationPhasedArray(){
    ...
    if (InstalledStations.size() > 1){ // Station 1
        ...
        //        Antennas.push_back(5);Antennas.push_back(9);Antennas.push_back(1);Antennas.push_back(17);
        Antennas.push_back(1);Antennas.push_back(2);Antennas.push_back(3);Antennas.push_back(4); Antennas.push_back(5);
        ...
        */
        ...
        InstalledStations[1].nChannels = 7;
        InstalledStations[1].nChannelsVH = 7;
...
int Detector::GetChannelfromStringAntenna ( int stationNum, int stringnum, int antennanum, Settings *settings1) {
    ...
    // if only ideal stations are in use and also installed ARA1a (use ARA1a ch mapping for now)
        ...
        if (stringnum < int(InstalledStations[1].VHChannel.size())){
            //cout<<"Antenna number :"<<int(InstalledStations[1].VHChannel[stringnum].size())<<endl;
