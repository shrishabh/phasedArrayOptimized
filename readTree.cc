...
cout<<"argc "<<argc<<endl;
  ...
  // }
  else { // Add all the root files into a chain for further analysis
...
chain.GetEntry(0);
   ...
   int numEntries = chain2.GetEntries();
...
//  }
  ...
  int num_plots = 10;
    ...
    ofstream myfile;
  	myfile.open ("data_test_sn.txt");

  for (int inu=0;inu<numEntries;inu++) { // loop over neutrinos
